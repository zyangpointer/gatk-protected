...
public class AllelicCountWithPhasePosteriorsReader extends TableReader<AllelicCountWithPhasePosteriors> {
    ...
    public AllelicCountWithPhasePosteriorsReader(final File file) throws IOException {
        ...
        if (columns().containsAll(PhasePosteriorsTableColumn.appendPhasePosteriorColumns(AllelicCountTableColumn.BASIC_COLUMNS).names())) {
        ...
        } else if (columns().containsAll(PhasePosteriorsTableColumn.appendPhasePosteriorColumns(AllelicCountTableColumn.FULL_COLUMNS).names())) {
    ...
    protected AllelicCountWithPhasePosteriors createRecord(final DataLine dataLine) {
        ...
        final double refMinorProb = dataLine.getDouble(PhasePosteriorsTableColumn.REF_MINOR_PROB);
        final double altMinorProb = dataLine.getDouble(PhasePosteriorsTableColumn.ALT_MINOR_PROB);
        final double outlierProb = dataLine.getDouble(PhasePosteriorsTableColumn.OUTLIER_PROB);
